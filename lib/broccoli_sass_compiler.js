...
BroccoliSassCompiler.prototype.build = function() {
  ...
  if (this.hasKnownDependencies()) {
    //console.log("%%% Has known dependencies");
    ...
    //console.log("%%% No known dependencies");
  }
  ...
  treeFiles = treeFiles.filter(function(f, i) {
    ...
    if (self.dependencies[f] === undefined) {
      //console.log("building because no deps for", f);
      return true;
    }
    for (var p = 0; p < patches.length; p++) {
      var entry = patches[p][2];
      if (self.dependencies[f].has(entry.relativePath)) {
        //console.log("building because", entry.relativePath, "is used by", f);
        return true;
      }
    }
  });
  //console.log("building files:", treeFiles);

  this.clearDependencies(treeFiles.map(function(f) {
