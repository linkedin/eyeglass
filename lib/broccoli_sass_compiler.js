...
BroccoliSassCompiler.prototype.keyForTreeSource = function(srcDir, relativeFilename, options) {
  var absolutePath = path.join(srcDir, relativeFilename);
  return readFile(absolutePath, READ_AS_UTF_8).then(function(data) {
    return relativeFilename + "@" + crypto.createHash("md5").update(data).digest("hex");
  });
};
...
BroccoliSassCompiler.prototype.compileCssFileMaybe = function(details) {
  ...
  if (cache) {
    return this.keyForTreeSource(details.srcPath, details.sassFilename, details.options).
      then(function(key) {
        debugger;
        return cache.get(key).
          then(function(cacheEntry) {
            if (cacheEntry.isCached) {
              debug("persistent cache", details.sassFilename,
                    "is cached. writing to", details.fullCssFilename);
              return mkdirp.promise(path.dirname(details.fullCssFilename)).
                then(function() {
                  return writeFile(details.fullCssFilename, cacheEntry.value).finally(function() {
                  });
                });
            } else {
              debug("persistent cache", details.sassFilename, "is not cached. Compiling.");
