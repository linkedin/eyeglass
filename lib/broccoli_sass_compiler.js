...
module.exports = class BroccoliSassCompiler extends BroccoliPlugin {
  ...
  compileTree(srcPath, files, destDir) {
    switch (files.length) {
      ...
      default:  {
        let numConcurrentCalls;

        if ("CONCURRENT_BROCCOLI_EYEGLASS" in process.env) {
          numConcurrentCalls = Number(process.env.JOBS) || os.cpus().length;
        } else {
          numConcurrentCalls = 1;
        }
  ...
  cachedOutput(srcDir, relativeFilename, options) {
    let key = this.keyForSourceFile(srcDir, relativeFilename, options);
    let cacheEntry = this.persistentCache.get(this.dependenciesKey(key));
    if (cacheEntry.isCached) {
      let dependencies = JSON.parse(cacheEntry.value);

      // throws if a dep is missing
      dependencies.map(dep => this.checkDependency(srcDir, dep));

      try {
        let cacheEntry = this.persistentCache.get(this.outputKey(key));
        if (cacheEntry.isCached) {
  ...
  compileCssFileMaybe(details) {
    if (this.persistentCache) {
      try {
        let value = this.cachedOutput(details.srcPath, details.sassFilename, details.options);
        return this.handleCacheHit(details, value);
      } catch (e) {
        return this.handleCacheMiss(details, e);
      }
    } else {
      return this.compileCssFile(details);
    }
