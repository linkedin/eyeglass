  ...
  return new RSVP.Promise(function(resolve, reject) {
    self.keyForSourceFile(srcDir, relativeFilename, options).then(function(key) {
      if (self.buildCount === 1 && process.env["BROCCOLI_EYEGLASS"] === "forceInvalidateCache") {
        var error = new Error("Environment variable forced invalidation of " + relativeFilename);
        error.key = key;
        reject(error);
      }
...
BroccoliSassCompiler.prototype.compileCssFileMaybe = function(details) {
  ...
  if (cache) {
    return this.cachedOutput(details.srcPath, details.sassFilename, details.options).
      then(function cachHit(inputAndOutputFiles) {
             ...
             var results = [];
             self.dependencies[details.fullSassFilename] = new Set();
             self.outputs[details.fullSassFilename] = new Set();
             inputFiles.forEach(function(dep) {
               // populate the dependencies cache for rebuilds
               self.dependencies[details.fullSassFilename].add(
                 path.resolve(details.srcPath, dep));
             });
             ...
             files.forEach(function(f) {
               ...
               self.outputs[details.fullSassFilename].add(outputFile);
             ...
             return self.compileCssFile(details).
               then(function(result) {
                 if (key) {
                   ...
                   if (self.dependencies[details.fullSassFilename]) {
                     self.dependencies[details.fullSassFilename].forEach(function(f) {
                       dependencyPromises.push(self.hashForFile(f).then(function(h) {
                         ...
                         if (e.code === "ENOENT") {
                           debug("Ignoring missing dependency %s", f);
                           return [];
                         } else {
                           throw e;
                         }
                   ...
                   var setDeps = RSVP.all(dependencyPromises).then(function(depsWithHashes) {
                     depsWithHashes = depsWithHashes.filter(function(dwh) {
                       return dwh.length > 0;
                     });
                     return cache.set(self.dependenciesKey(key), JSON.stringify(depsWithHashes));
                   });
                   var outputs = self.outputs[details.fullSassFilename];
                   outputContents = {};
                   outputContents[details.cssFilename] = new Buffer(result.css).toString("base64");
                   if (outputs) {
                     outputs.forEach(function(o) {
                       if (o.startsWith(details.destDir)) {
                         outputContents[o.substring(details.destDir.length + 1)] =
                           fs.readFileSync(o).toString("base64");
                       } else {
                         debug("refusing to cache output file found outside the output tree: %s", o);
                       }
                     });
                   }
                   var setOutput = cache.set(self.outputKey(key), JSON.stringify(outputContents));
