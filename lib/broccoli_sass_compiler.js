...
var BroccoliSassCompiler = CachingWriter.extend({
  ...
  compileCssFile: function(details) {
    var self = this;
    var promise = this.events.emit("compiling", details);
    var compiled = promise.then(function() {
      var compile = renderSass(details.options);
      var handled = compile.then(self.handleSuccess.bind(self, details),
                                 self.handleFailure.bind(self, details));
      return RSVP.allSettled([compile, handled]);
    });
    return RSVP.allSettled([promise, compiled]);
  },
