    ...
    return this.cachedOutput(details.srcPath, details.sassFilename, details.options).
      then(function cachHit(output) {
             ...
             return mkdirp.promise(path.dirname(details.fullCssFilename)).
               then(function() {
                 return writeFile(details.fullCssFilename, output);
               });
           },
           function cacheMiss(reason) {
             ...
             return self.compileCssFile(details).
               then(function(result) {
                 if (key) {
                   var dependencyPromises = result.stats.includedFiles.sort().map(function(f) {
                     return self.hashForFile(f).then(function(h) {
                   ...
                   var setOutput = cache.set(self.outputKey(key), result.css);
