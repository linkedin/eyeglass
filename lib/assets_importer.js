...
function makeImporter(eyeglass, sass, options, fallbackImporter) {
  ...
  function getModuleByName(moduleName, dir) {
    ...
    for (var i = 0; i < allModules.length; i++) {
      if (moduleName === allModules[i].eyeglassName) {
  ...
  return function(uri, prev, done) {
      ...
      if (isRealFile) {
        ...
        if (pkgRootDir && pkgRootDir !== root) {
          ...
          if (eyeglassExports.assets) {
            ...
            done(sass.types.Error("No assets specified for plugin " + egModDef.eyeglassName));
          }
        } else {
    ...
    if (jsFile) {
      ...
      if (mod.assets) {
        ...
        done(sass.types.Error("No assets specified for eyeglass plugin " + moduleName));
      }
    } else {
      done(sass.types.Error("No eyeglass plugin named: " + moduleName));
    }
  };
}
