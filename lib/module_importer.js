...
function readAbstractFile(uri, origin, location, includePaths, cb) {
  ...
  for (var i in includePaths) {
    thisLocation = path.resolve(location, includePaths[i]);
    possibleNames = possibleNames.concat(efs.expandFileName(uri, origin, thisLocation));
  }
...
function makeImporter(eyeglass, sass, options, fallbackImporter) {
  ...
  return function(uri, prev, done) {
    ...
    var includePaths = options.includePaths.split(path.delimiter); // surprised this is not already an array, coming from broccoli-eyeglass
