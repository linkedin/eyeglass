 ...
 */
function readFirstFile(possibleFiles, doneError, cb) {
  var nextFile = possibleFiles.next().value;
  if (nextFile) {
    fileUtils.readFile(nextFile, "utf8", function(err, data) {
      if (err) {
        readFirstFile(possibleFiles, doneError, cb);
      } else {
        cb(null, {
...
function readAbstractFile(originalUri, uri, location, includePaths, moduleName, cb) {
  ...
  readFirstFile(possibleFiles, new Error(
    "Could not import " + originalUri + " from any of the following locations:\n  " +
    nameExpander.getFilesAsArray().join("\n  ")
  ), cb);
}
