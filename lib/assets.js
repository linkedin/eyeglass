...
AssetPathEntry.prototype = {
  ...
  asRegistrationString: function(defaultName) {
    ...
    files.forEach(function(file) {
      ...
      assetMap.push('"' + file + '": (filepath: "' + path.join(self.srcPath, file) + '", uri: "' + uri + '")');
    });
...
function resolveAsset(sass, sassUtils, registeredAssetsMap, relativePathString) {
...
// XXX I think it's strange this isn't async like resolveAssetToUri.
function resolveAssetToPath(sass, sassUtils, registeredAssetsMap, relativePathString) {
  var data = resolveAsset(sass, sassUtils, registeredAssetsMap, relativePathString);

  if (data) {
    return data.coerce.get("filepath");
  }
}

function resolveAssetToUri(eyeglass, sass, sassUtils, registeredAssetsMap, relativePathString, cb) {
  var data = resolveAsset(sass, sassUtils, registeredAssetsMap, relativePathString);

  if (data) {
    var uri = data.coerce.get("uri");
    var filepath = data.coerce.get("filepath");
    var globalHttpPrefix = eyeglass.options.assetsHttpPrefix ?
                         "/" + eyeglass.options.assetsHttpPrefix : "";
    var fullUri = globalHttpPrefix + "/" + uri;
...
module.exports = function(eyeglass, sass) {
  ...
  mainCollection.resolveAssetToUri = function(registeredAssetsMap, relativePathString, cb) {
    resolveAssetToUri(eyeglass, sass, sassUtils, registeredAssetsMap, relativePathString, cb);
  };
