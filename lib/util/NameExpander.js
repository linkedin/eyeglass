...
NameExpander.prototype.getFiles = function() {
  var files = [];
  this.set.forEach(function(file) {
    files.push(file);
  });
  return files;
};
  ...
  if (location && location !== "stdin" && fileUtils.existsSync(location)) {
    ...
    if (!rIgnoredExtensions.test(path.extname(this.uri))) {
      ...
      withVariants(basename, function (variantName) {
        ...
        if (dir && variantName) {
          this.set.add(path.join(dir, variantName));
        }
      }.bind(this));
      ...
      withVariants("index", function (variantName) {
        this.set.add(path.join(fullLocation, variantName));
      }.bind(this));
    }
  }
};
